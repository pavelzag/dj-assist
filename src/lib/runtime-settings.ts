...
export type PendingCloudAuthSession = PendingGoogleAuthSession & {
...
export async function saveDropboxAuth(auth: Omit<CloudAuthSettings, 'provider' | 'updatedAt'>): Promise<CloudAuthSettings> {
  ...
  await saveRuntimeSettings({ ...current, dropboxAuth: next });
  return next;
}
...
export async function clearAuthSettingsForProvider(provider: CloudAuthProvider): Promise<void> {
  const current = await loadRuntimeSettings();
  const next = { ...current };
...
function authScopes(auth: Pick<AuthSettings, 'scopes'> | Pick<CloudAuthSettings, 'scopes'>): string[] {
...
export async function getDropboxAccessToken(): Promise<{
  ...
  const settings = await loadRuntimeSettings();
  let auth = settings.dropboxAuth;
  if (!auth || auth.provider !== 'dropbox' || !auth.id) {
    throw new Error('Dropbox sign-in is required before importing from Dropbox.');
  }
  if (!auth.accessToken || !auth.accessTokenExpiresAt || Date.parse(auth.accessTokenExpiresAt) <= Date.now() + 30_000) {
  ...
  if (!auth.accessToken) {
    throw new Error('Dropbox access token is unavailable. Sign in again and retry the import.');
  }
  return { accessToken: auth.accessToken, auth };
}
...
export async function serverRuntimeSummary() {
  ...
  return {
    ...
    dropbox: {
      ...
      hasRefreshToken: Boolean(dropboxAuth?.refreshToken),
      scopes: dropboxAuth ? authScopes(dropboxAuth) : [],
    },
  };
}
...
export function applyDropboxOauthCredentialsToEnv(credentials: Partial<DropboxOauthCredentials> | null | undefined) {
  const clientId = String(credentials?.clientId ?? '').trim();
  const clientSecret = String(credentials?.clientSecret ?? '').trim();
  if (clientId) process.env.DROPBOX_CLIENT_ID = clientId;
  else delete process.env.DROPBOX_CLIENT_ID;
  if (clientSecret) process.env.DROPBOX_CLIENT_SECRET = clientSecret;
  else delete process.env.DROPBOX_CLIENT_SECRET;
}
...
export async function effectiveDropboxOauthCredentials() {
  return effectiveProviderOauthCredentials('dropbox');
}
